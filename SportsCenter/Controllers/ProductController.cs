...
namespace SportsCenter.Controllers
{
    public class ProductController : Controller
    {
        ...
        {
            var badminton = (from a in _dbContext.Products where a.Item_Id == 4 select a);
            return badminton;
        }
        [HttpGet]
        public IEnumerable<Products> TableTennisView()
        {
            var tableTennis = (from a in _dbContext.Products where a.Item_Id == 5 select a);
            return tableTennis;
        }
        [HttpGet]
        public IEnumerable<Products> PoolView()
        {
            var pool = (from a in _dbContext.Products where a.Item_Id == 6 select a);
            return pool;
        }
        [HttpGet]
        public IEnumerable<Products> SquashView()
        {
            var squash = (from a in _dbContext.Products where a.Item_Id == 7 select a);
            return squash;
        }
